...
export class TimeSelectService {
    ...
    getHours(config: ITimeSelectConfigInternal, t: Moment | null): string {
        const time = t || moment();
        let val = time && time.format(config.showTwentyFourHours ? config.hours24Format : config.hours12Format);
        return config.locale === 'fa' ? this.utilsService.toPersian(val) : val;
    }

    getMinutes(config: ITimeSelectConfigInternal, t: Moment | null): string {
        const time = t || moment();
        let val = time && time.format(config.minutesFormat);
        return config.locale === 'fa' ? this.utilsService.toPersian(val) : val;
    }

    getSeconds(config: ITimeSelectConfigInternal, t: Moment | null): string {
        const time = t || moment();
        let val = time && time.format(config.secondsFormat);
        return config.locale === 'fa' ? this.utilsService.toPersian(val) : val;
    }
